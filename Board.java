...
public class Board extends Group {
  ...
  private final BooleanProperty gameBotProperty = new SimpleBooleanProperty(false);
  ...
  private final Button bActeveBot = new Button("Continue");
  private final Button bContinueNo = new Button("No, continue");
  ...
  private void createScore() {
    Label lblTitle = new Label("Game 2048");
  ...
  private Rectangle createCell(int PosX, int PosY) {
    final double arcSize = CELL_SIZE / 6d;
    Rectangle cell = new Rectangle(PosX * CELL_SIZE, PosY * CELL_SIZE, CELL_SIZE, CELL_SIZE);
  ...
  private void createGrid() {
    gridOperator.traverseGrid((PosX, PosY) -> {
      gridGroup.getChildren().add(createCell(PosX, PosY));
      return 0;
    });
  ...
  private void initGameProperties() {
    ...
    gameOverProperty.addListener(
        new Overlay("Game over!", "", bTry, null, "game-overlay-over", "game-lblOver", false));
    gameBotProperty.addListener(new Overlay("Bot actevate?", "", bActeveBot, null,
        "game-overlay-pause", "game-lblPause", true));
    gamePauseProperty.addListener(new Overlay("Game Paused", "", bContinue, null,
        "game-overlay-pause", "game-lblPause", true));
    gameTryAgainProperty.addListener(new Overlay("Try Again?", "Current game will be deleted", bTry,
        bContinueNo, "game-overlay-pause", "game-lblPause", true));
